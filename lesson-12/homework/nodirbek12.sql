...
6)SELECT 
    CustomerID,
    SUM(CASE WHEN Weight > 50 THEN TotalAmount ELSE 0 END) AS TotalSalesAbove50,
    MIN(Weight) AS LeastWeight
FROM Sales.Orders
GROUP BY CustomerID;

7)DROP TABLE IF EXISTS Cart1;
...
8)Create table Customers (id int, name varchar(255))
...
9)Create table Students (student_id int, student_name varchar(20))
