...
5) select* from Products_Discounted
order by Price asc

6) select* from Employees
where Salary >= 6000 and DepartmentName = 'hr'

7) SELECT EmployeeID, FirstName, ISNULL(Email, 'noemail@example.com') AS Email
FROM Employees

8) select* from Products_Discounted
where Price > 50 and Price < 100 

9) SELECT DISTINCT Category, ProductName FROM Products_Discounted

10) SELECT* FROM Products_Discounted
...
12) select EmployeeID, coalesce( FirstName, LastName, 'notnull') from Employees
  
13) select distinct Category, Price from Products_Discounted
...
16)select* from Products_Discounted
where Price <=1000 and StockQuantity >50
order by StockQuantity asc

17) select* from Products_Discounted
