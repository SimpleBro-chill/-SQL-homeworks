...
3)select count(*)  as R_ow from Customers

4) select count(  distinct Category) as unique_category from Products

5)SELECT SUM(SaleAmount) AS TotalSales FROM Sales
WHERE ProductID = 7;

6) select avg(Age) from Employees

7)select count(*) as number_employees from Employees
...
9) select sum(SaleAmount) as PricePER_person from Sales
group by CustomerID
...
12) SELECT COUNT(*) AS NumberOfEmployees
FROM Employees
...
14) select avg(Salary) as AVG_salary from 
...
16) select  ProductName, 
 avg(Price) as avg_price from Products
 group by ProductName
 having avg(Price)> 400

17) select sum(SaleAmount) as total_sales  from Sales
group by SaleDate

18) select CustomerID, count(Quantity) from Orders
group by CustomerID
having count(Quantity)>=3

19) select EmployeeID, avg(Salary)  from Employees
group by EmployeeID
