...
1) SELECT Category, COUNT(*)  AS TOTAL_NUMBER FROM Products
GROUP BY Category

2) SELECT Category, AVG(Price) AS AVG_PRICE FROM Products
GROUP BY Category
...
8) select count(CustomerID) as total_number from Customers
...
16) select SaleID , sum(SaleAmount) as total_sales from Sales
group by SaleID
